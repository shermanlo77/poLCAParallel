...
class EmAlgorithm {
 ...
 public:
  ...
  virtual ~EmAlgorithm() { delete[] this->ln_l_array_; }
  ...
  void Fit() {
    ...
    while (!is_success) {
      ...
      is_success = true;
      for (this->n_iter_ = 0; this->n_iter_ <= this->max_iter_;
           ++this->n_iter_) {
